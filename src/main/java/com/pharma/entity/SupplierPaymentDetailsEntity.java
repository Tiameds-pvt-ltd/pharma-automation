package com.pharma.entity;

import jakarta.persistence.*;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;
import org.hibernate.annotations.GenericGenerator;

import java.math.BigDecimal;
import java.time.LocalDate;
import java.util.UUID;

@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
@Entity
@Table(name="pharma_supplier_payment_details")
public class SupplierPaymentDetailsEntity {

    @Id
    @GeneratedValue(generator = "UUID")
    @GenericGenerator(name = "UUID", strategy = "org.hibernate.id.UUIDGenerator")
    @Column(name = "payment_details_id", updatable = false, nullable = false, unique = true)
    private UUID paymentDetailsId;

    @Column(name = "purchase_bill_no")
    private String purchaseBillNo;

    @Column(name = "cleared_amount")
    private BigDecimal clearedAmount;

    @Column(name = "created_by")
    private Long createdBy;

    @Column(name = "created_date")
    private LocalDate createdDate;

    @Column(name = "modified_by")
    private Long modifiedBy;

    @Column(name = "modified_Date")
    private LocalDate modifiedDate;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "payment_id", nullable = false)
    private SupplierPaymentEntity supplierPaymentEntity;

    @PrePersist
    public void generateUUID() {
        if (paymentDetailsId == null) {
            paymentDetailsId = UUID.randomUUID();
        }
    }
}
